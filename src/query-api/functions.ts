...
import { CacheAPIFunctionsInvitations } from "@/query-api/protocol/invitations";
...
import { CacheAPIFunctionsOther } from "@/query-api/protocol/other";
...
export const CacheAPIFunctions: Record<CacheKeyEntity, (args: unknown[]) => Promise<CacheValue>> = {
    ...
    ...CacheAPIFunctionsInvitations,
    ...CacheAPIFunctionsOther,
};
...
export const resolveAPIFunction = async (options: { key: CacheKey }) => {
    ...
    if (!handler) {
        throw new Error(`No handler defined for entity: ${entity} with args: ${args}`);
    }
