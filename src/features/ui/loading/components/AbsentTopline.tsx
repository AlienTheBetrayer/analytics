...
export const AbsentTopline = ({ title, color, className }: Props) => {
    return (
        <ul
            className={`box loading p-0! sticky! z-2! top-4! rounded-full! px-1! gap-1! my-4! mx-auto! flex-row! max-w-400 w-full transition-all duration-500 h-10 items-center ${className ?? ""}`}
        >
            <li>
                <motion.ul
                    initial={{ opacity: 0 }}
