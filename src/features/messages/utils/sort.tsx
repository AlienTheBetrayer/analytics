...
export const sortConversations = (options: {
    conversations: MapType<CacheAPIProtocol["conversations"]["data"]>[];
    reversed?: boolean;
    filter?: string;
}) => {
    const elements = Object.values(options.conversations)
        .sort((a, b) => {
 ...
 * @returns copied object
 */
export const sortMessages = <T extends { message: string; created_at: string }>(options: {
    messages: T[];
    filter?: string;
    reversed?: boolean;
}) => {
